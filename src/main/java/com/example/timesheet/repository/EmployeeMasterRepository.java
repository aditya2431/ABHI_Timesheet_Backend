...
public interface EmployeeMasterRepository extends JpaRepository<EmployeeMaster, Integer> {
	...
	@Query(value = "SELECT DISTINCT DATE_FORMAT(all_dates.date, '%Y-%m-%d') AS missing_date " +
            "FROM (" +
            ...
            "WHERE MONTH(all_dates.date) = MONTH(CURDATE()) " +
            "AND YEAR(all_dates.date) = YEAR(CURDATE()) " +
